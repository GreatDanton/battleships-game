$('document').ready(function() {
  ...
  $('#userships .square').click(function() {
    ...
    var currentShipCoordinates = ids.replace(/#userships /gi, '');
    var takenPositions = userShipPositions.join('').split(',');

    if (takenPositions.length > 1) {
      for (i = 0; i < takenPositions.length; i++) {
        if (currentShipCoordinates.indexOf(takenPositions[i]) > -1) {
          ids += 'undefined';
        }
      }
    }
